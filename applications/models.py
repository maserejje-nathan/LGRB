...
def generate_principle_random_string():
...
def generate_employee_random_string():
    ...
    digits = random.randint(1, 999)
    # Combine the elements into a string
    return f"{employee_prefix}-{number:02d}-{digits:03d}"


def generate_premise_random_string():
    ...
    digits = random.randint(1, 999)
    # Combine the elements into a string
    return f"{premise_prefix}-{number:02d}-{digits:03d}"


def generate_principle_licence_number(instance):
    new_certificate_number = generate_principle_random_string()
    jclass = instance.__class__
    exists = jclass.objects.filter(certificate_number = new_certificate_number).exists()
    if exists:
        return generate_principle_random_string()
    return new_certificate_number 


def generate_employee_licence_number(instance):
    new_certificate_number = generate_employee_random_string()
    jclass = instance.__class__
    exists = jclass.objects.filter(certificate_number = new_certificate_number).exists()
    if exists:
        return generate_employee_random_string()
    return new_certificate_number


def generate_premise_licence_number(instance):
    new_certificate_number = generate_premise_random_string()
    jclass = instance.__class__
    exists = jclass.objects.filter(certificate_number = new_certificate_number).exists()
    if exists:
        return generate_premise_random_string()
    return new_certificate_number



def pre_save_principle_certificate_number(sender, instance, *args, **kwargs):
...
class PrincipleLicence(models.Model):
    ...
    memorandum_and_articles_of_association = models.FileField(upload_to='media/files/memorandum_and_articles_of_association', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['pdf'])])
    coperate_social_responsibility = models.FileField(upload_to='media/files/coperate_social_responsibility', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['pdf'])])
    districts_of_conduction = models.FileField(upload_to='media/files', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['pdf'])])
    annual_company_returns = models.FileField(upload_to='media/files/annual_company_returns', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['pdf'])])
    tenancy_agreement = models.FileField(upload_to='media/files/tenacy_agreements', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['pdf'])])
    categories_of_machines = models.FileField(upload_to='media/files/categories_of_machines', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['pdf'])])
    ...
    def get_absolute_url(self, *args, **kwargs):
        return reverse('applications:principle-detail', kwargs= {'id': self.id} )

    def get_prn_url(self, *args, **kwargs):
        return reverse('applications:get_prn', kwargs= {'id': self.id} )

    def get_update_url(self):
        return reverse("applications:principle-update", kwargs={
            'id': self.id
        })


pre_save.connect(pre_save_principle_certificate_number, sender=PrincipleLicence)



def pre_save_update_employee_licence_certificate_number(sender, instance, *args, **kwargs):
    if not instance.certificate_number:
        instance.certificate_number = generate_employee_licence_number(instance)
...
class EmployeeLicence(models.Model):
    ...
    def get_employee_detail_url(self, *args, **kwargs):
        return reverse('applications:employee-detail', kwargs= {'pk': self.pk} )
    
    def get_employee_update_url(self, *args, **kwargs):
        return reverse('applications:employee-update', kwargs= {'pk': self.pk} )
...
pre_save.connect(pre_save_update_employee_licence_certificate_number, sender=EmployeeLicence)



def pre_save_update_premise_licence_certificate_number(sender, instance, *args, **kwargs):
    if not instance.certificate_number:
        instance.certificate_number = generate_premise_licence_number(instance)
...
pre_save.connect(pre_save_update_premise_licence_certificate_number, sender=PremiseLicence)
